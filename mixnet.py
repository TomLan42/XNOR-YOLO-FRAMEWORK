...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        ...
        if str(v)[-1] == 'B':
            cout = int(v[:-1])
            layers += [BinConv2d(in_channels,cout,kernel_size=3, stride=s, padding=1, groups=1)]
            in_channels = cout
            if first_bin_flag:
                bin_range[0] = pointer
            ...
            in_channels = v
            pointer += 1
    bin_range[1] = pointer-1
    return nn.Sequential(*layers),bin_range
...
def vgg16_mix3(pretrained=False, **kwargs):
    """VGG 16-lyaer model, last 3 layer binarized 

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    features,bin_range = make_layers(cfg['F'], batch_norm=False)
    model = VGG(features, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['vgg16']))
    return model,bin_range
